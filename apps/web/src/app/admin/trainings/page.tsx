...
import { adminApi, type AdminTraining, type TrainingListResponse } from "@/lib/admin-api"
import { Search, Loader2, ChevronLeft, ChevronRight, Plus, Edit, Trash2, Check, X, Archive, Eye, Star, Clock, Building2, MapPin, Calendar, Globe } from "lucide-react"
...
export default function AdminTrainingsPage() {
  ...
  const [data, setData] = useState<TrainingListResponse | null>(null)
  ...
  }, [q, status, providerId, isFeatured, page, sort])
  ...
  useEffect(() => {
    ...
    router.replace(qs ? `/admin/trainings?${qs}` : "/admin/trainings", { scroll: false })
  }, [q, status, providerId, isFeatured, router])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Təlimlər</h1>
          <p className="mt-1 text-sm text-slate-500">
            {data ? `Cəmi ${data.total} təlim` : "Yüklənir..."}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Ad, təlimçilər, əlavə axtar..."
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            <option value="PUBLISHED">Yayımlandıb</option>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-400">
                  <th className="px-6 py-3.5 font-semibold">Təlim</th>
                  <th className="px-4 py-3.5 font-semibold">Təlimçi</th>
                  <th className="px-4 py-3.5 font-semibold">Format</th>
                  <th className="px-4 py-3.5 font-semibold">Qiymət</th>
                ...
                {data.items.map((item: any) => (
                  <tr key={item.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                    <td className="max-w-[280px] px-6 py-4">
                      ...
                      <div className="mt-0.5 text-xs text-slate-400">{item.provider_name || "—"}</div>
                    </td>
                    <td className="px-4 py-4 text-slate-600">{item.provider_name || "—"}</td>
                    <td className="px-4 py-4 text-slate-600">{item.format ? FORMAT_LABELS[item.format] || item.format : "—"}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link href={`/admin/trainings/${item.id}`} className="text-xs font-semibold text-[#2563EB] hover:underline">
                        İdarə et →
                      </Link>
                    </td>
                  </tr>
