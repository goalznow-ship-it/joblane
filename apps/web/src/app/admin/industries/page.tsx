...
import { adminApi, type AdminIndustry, type IndustryListResponse, type IndustryUpdateRequest } from "@/lib/admin-api"
...
export default function AdminIndustriesPage() {
  ...
  const [data, setData] = useState<IndustryListResponse | null>(null)
  ...
  const [editData, setEditData] = useState<IndustryUpdateRequest>({})
  ...
  useEffect(() => {
    ...
    router.replace(qs ? `/admin/industries?${qs}` : "/admin/industries", { scroll: false })
  }, [q, isActive, router])
  ...
  const handleEdit = async (id: string) => {
    setEditError("")
    try {
      await adminApi.updateIndustry(id, editData)
      setEditOpen(null)
      setEditData({})
      fetchList()
    } catch (err) {
  ...
  const handleStatus = async (id: string, action: "activate" | "deactivate" | "archive") => {
    if (!confirm(`Sənayeni ${action === "activate" ? "aktiv" : action === "deactivate" ? "deaktiv" : "arxiv"} etmək istəyirsiniz?`)) return
    try {
      await adminApi.changeIndustryStatus(id, action)
      fetchList()
    } catch (err) {
      alert(String(err))
    }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Sənayeni silmək istəyirsiniz? Bu əməliyyat geri qaytarıla bilməz.")) return
    try {
      await adminApi.updateIndustry(id, { is_active: false })
      fetchList()
    } catch (err) {
      alert(String(err))
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Sənayelər</h1>
          <p className="mt-1 text-sm text-slate-500">
            {data ? `Cəmi ${data.total} sənaye` : "Yüklənir..."}
          </p>
        </div>
        <button
        ...
        >
          <Plus className="h-4 w-4" /> Yeni sənaye
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Sənaye adı, slug üzrə axtar..."
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-400">
                  <th className="px-6 py-3.5 font-semibold">Sənaye</th>
                  <th className="px-4 py-3.5 font-semibold">Slug</th>
                  <th className="px-4 py-3.5 font-semibold">Şirkətlər</th>
              ...
              <tbody>
                {data.items.map((ind: any) => (
                  <tr key={ind.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                    <td className="max-w-[280px] px-6 py-4">
                      <div className="truncate font-semibold text-slate-900">{ind.name}</div>
                    </td>
                    <td className="max-w-[180px] truncate px-4 py-4 text-slate-600">{ind.slug}</td>
                    <td className="px-4 py-4 text-slate-600">
                      {ind.total_companies_count !== null ? (
                        <>
                          <span className="font-semibold text-slate-900">{ind.total_companies_count || 0}</span>
                          <span className="text-slate-400"> / </span>
                          <span className="text-slate-500">{ind.total_jobs_count || 0}</span>
                        </>
                      ) : (
                        "—"
                      )}
                    ...
                    <td className="px-4 py-4">
                      <span className={cn("rounded-full px-2.5 py-1 text-[11px] font-semibold whitespace-nowrap", ind.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600")}>
                        {ind.is_active ? "Aktiv" : "Deaktiv"}
                      </span>
                    </td>
                    <td className="px-4 py-4 text-slate-500">{ind.sort_order}</td>
                    <td className="px-4 py-4 text-slate-500">{ind.created_at ? new Date(ind.created_at).toLocaleDateString("az-AZ") : "—"}</td>
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => handleStatus(ind.id, ind.is_active ? "deactivate" : "activate")}
                        className="text-xs font-semibold text-amber-600 hover:underline mr-2"
                      >
                        {ind.is_active ? "Deaktiv et" : "Aktiv et"}
                      </button>
                      <button
                        onClick={() => handleStatus(ind.id, "archive")}
      ...
      {createOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-6" onClick={() => setCreateOpen(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-slate-900">Yeni sənaye yarat</h3>
